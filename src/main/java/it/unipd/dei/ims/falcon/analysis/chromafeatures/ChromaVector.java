...
public class ChromaVector {
	 ...
	 * 
	 * @param k max ranks to consider - NOTE MUST BE leq 7
	 * @param npeaks actual number of ranks to use
	 */
	public int rankRepresentation(int k, int npeaks) {
		...
		for (int i = 0; i < npeaks; i++) {
			...
			representation += (largest + 1) * Integer.rotateLeft(1, 4 * (k - i - 1));
			chromacopy[largest] = -1;
		}
		rankRep = representation;
		return rankRep;
	}
	...
	public int rankRepresentation(double[] intervals) {
		return (intervals.length == 0)
            ? rankRepresentation(1)
            : rankRepresentation(intervals.length + 1, howmanypeaks(kurtosis, intervals));
	}
